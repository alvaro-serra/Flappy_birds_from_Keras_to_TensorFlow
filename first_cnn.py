...
def buildmodel():
	...
	with first_conv_net.as_default():
		...
		loss = tf.losses.mean_squared_error(label,scores,weights=1.0,scope=None,loss_collection=tf.GraphKeys.LOSSES,reduction=Reduction.SUM_BY_NONZERO_WEIGHTS)
	    ...
	    while (True):
	        ...
	        if t > OBSERVE:
	            ...
	            for i in range(0, len(minibatch)):
	                ...
	                feed_state_t = {image:state_t}
	                feed_state_t1 = {image:state_t1}
	            ...
	            feed_train = {image: inputs, label: targets}
        		_, current_loss = sess.run([train_step, loss], feed_dict=feed_train)
        		loss += current_loss
